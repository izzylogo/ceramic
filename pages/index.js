...
// Record Setter Component
function RecordSetter() {
  const record = useViewerRecord("basicProfile");
  const [name, setName] = useState("");
  const [age, setAge] = useState("")

  const updateRecordName = async (name, age) => {
    await record.merge({
      name: name,
      age: age,
    });
  ...
  return (
    <div className={styles.content}>
      <div className={styles.mt2}>
        {record.content ? (
          <div className={styles.flexCol}>
            ...
            <span className={styles.subtitle}>
              {record.content.name} is {record.content.age} years old
            </span>
      ...
      <input 
        ...
        className={styles.input}
      />
      <button className={styles.but} onClick={() => updateRecordName(name, age)}>Update</button>
    </div>
  );
}
